...
const ActorPage: NextPage<IActorPage> = ({ movies, actor }) => {
	return actor ? <Actor movies={movies} actor={actor} /> : <div>Not found</div>
}
...
export const getStaticPaths: GetStaticPaths = async () => {
	try {
		...
		const paths = actors.map(actor => ({
			...
			// eğer genreye id ile erişiyorsan, id ise params : { id : actor.id } şeklinde yazıyoruz.
			params: {
				slug: actor.slug
			}
		}))
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
	...
	try {
		...
		// actor._id ile actor id'sine göre filmleri getiriyoruz.
